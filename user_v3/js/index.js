if(common.getOauth("http://h5.snh48.com/snh48/user_v3")){
...
//添加事件
	function addListeners(){
		...
		$(".btn-cz").click(function(){
			...
			$(".cz-box").show();
			$(".btns, .topcontent, .tip, .btn-cz-qr").hide();
		})
		//验证口袋账号
		...
		$(".btn-game").click(function(){
			window.location.href = "https://h5.48.cn/2018apppage/20181111/"
		})
	}
	//获取个人信息
	function loaduser(){
	...
	function appUserConfirm(){
		...
		$.ajax({
			type:'post',
			url:CONFIG.getLink()+"pay/app_user_confirm.php",
			data:{
			...
			success:function(rs){
				if(rs.errcode == "0"){//验证通过，跳到充值页
					...
					$(".qr-nickname").find("img").attr("src","http://source.snh48.com"+rs.avatar);
					//显示头像 隐藏梦想信息
					$(".num, .tit").hide();
					$(".headimg img").attr("src", "http://source.snh48.com"+rs.avatar);
					$(".meng .titname").html(rs.nick_name);
	...
	function getItemSaleList(){
		$.ajax({
			...
			success:function(data){
				...
				html += '<li sale_id="0"><input class="money" type="tel" placeHolder="输入500以上面额""/><span>购买</span></li>'
	...
	function getOrderList(_type,_sale_id,_pay_amount){
		...
		$.ajax({
			type:'get',
			url:CONFIG.getLink()+"pay/h5_weixin/pay.php?sale_id="+_sale_id+"&user_id="+$(".pocketid").val()+"&pay_amount=1&openid="+openid+"&ticket_type="+_type+"&custom_price="+_pay_amount,
			success:function(rs){
				//if(rs.prepay_id !=""){
